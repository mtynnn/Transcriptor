...
declare global {
  interface Window {
    ...
    pywebview: {
      api: {
        open_file_dialog: (title: string, fileTypes: string) => Promise<string | null>;
      };
    };
  }
}
...
export default function App() {
  ...
  const [transcriptionTitle, setTranscriptionTitle] = useState('Nueva Transcripción');
  ...
  // Library States
  const [selectedLibraryCategory, setSelectedLibraryCategory] = useState<string>('General');
  const [libraryMonth, setLibraryMonth] = useState<number>(new Date().getMonth());
  const [libraryYear, setLibraryYear] = useState<number>(new Date().getFullYear());
  const [selectedLibraryDay, setSelectedLibraryDay] = useState<number | null>(null);
  ...
  const [driveBreadcrumbs, setDriveBreadcrumbs] = useState<any[]>([{id: 'root', name: 'Mi Unidad'}]);
  const [selectedDriveAccount, setSelectedDriveAccount] = useState('');
  const [isDriveLoading, setIsDriveLoading] = useState(false);
  const [driveTargetType, setDriveTargetType] = useState<'audio' | 'context'>('audio');
    ...
    const startBackend = async () => {
      if (window.__TAURI_INTERNALS__) {
        try {
          ...
          // Nota: En producción, Tauri busca el binario en la carpeta de recursos configurada
          const command = Command.create('resources/engine/vtranscriptor-engine.exe');
    ...
    const loadPersistedSettings = async () => {
      try {
        ...
        if (histData && Array.isArray(histData)) {
           ...
           const cats = Array.from(new Set(histData.map((h: any) => h.category || 'General')));
           if (cats.length > 0) setAvailableCategories(prev => Array.from(new Set([...prev, ...cats as string[]])));
        }
      } catch (e) {
  ...
  const wordsCount = transcription.trim() === "" ? 0 : transcription.split(/\s+/).length;
  ...
  const confirmLinkDriveAccount = async () => {
    ...
    try {
      const res = await fetch(`http://localhost:8000/drive/auth?account_name=${encodeURIComponent(accountNameInput.trim())}`, {
        method: 'POST'
      });
      const data = await res.json();
      if (data.success) {
  ...
  const libraryItemsForSelectedMonth = historyItems.filter((item: any) => {
    ...
    return dateObj.getMonth() === libraryMonth && dateObj.getFullYear() === libraryYear;
  });
  ...
  const renderCalendarCells = () => {
    ...
    for (let d = 1; d <= daysInMonth; d++) {
      ...
      cells.push(
        <div 
          key={`day-${d}`} 
          onClick={() => hasItems && setSelectedLibraryDay(isSelected ? null : d)}
          className={cn(
            "aspect-square rounded-xl flex flex-col items-center justify-center relative transition-all duration-300",
            hasItems ? "cursor-pointer hover:border-[var(--primary)] border-2" : "opacity-30 border border-transparent",
            isSelected ? "bg-[var(--primary)] bg-opacity-20 border-[var(--primary)] text-[var(--primary)] font-bold shadow-lg scale-110 z-10" : "border-[var(--border)] bg-[var(--secondary)]"
          )}
        >
          ...
          {hasItems && (
            <div className="absolute bottom-2 flex gap-1">
              {dayItems.slice(0, 3).map((_, idx) => (
                <div key={idx} className="w-1.5 h-1.5 rounded-full bg-[var(--primary)] shadow-[0_0_5px_rgba(var(--primary-rgb),0.8)]"></div>
              ))}
              {dayItems.length > 3 && <div className="w-1.5 h-1.5 rounded-full bg-[var(--primary)] opacity-50"></div>}
            </div>
          )}
        </div>
      );
  ...
  return (
    <div className={cn("flex flex-col h-screen overflow-hidden transition-colors duration-500", `theme-${theme}`, "bg-[var(--background)] text-[var(--foreground)]")}>
      ...
      <header className="flex items-center justify-between px-8 py-4 glass-morphism border-b border-[var(--border)] bg-[var(--header)] z-50">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h1 className="text-xl font-bold tracking-tight text-[var(--foreground)] leading-tight text-left">vTranscriptor</h1>
            <p className="text-[10px] text-[var(--primary)] font-bold uppercase tracking-wider text-left">AI Local para Veterinaria</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-[var(--card)] border border-[var(--border)]">
            ...
            <span className="text-xs font-medium text-[var(--foreground)]">GPU ACTIVE</span>
          </div>
        </div>
      </header>
      ...
      <main className="flex-1 flex overflow-hidden relative">
        ...
        <aside className="w-20 lg:w-64 border-r border-[var(--border)] flex flex-col pt-8 bg-[var(--sidebar)] z-40">
            {[
              ...
              const isCompleted = (step.id === 1 && audioPath) || (step.id === 2 && transcription);
              ...
              return (
                ...
                >
                  {step.icon}
                  <span className="hidden lg:block font-medium text-sm text-left">{step.label}</span>
                  {isCompleted && <CheckCircle2 size={14} className="ml-auto text-green-500" />}
                </div>
              );
            })}
        </aside>
          ...
          <div className="max-w-4xl mx-auto space-y-10 text-left">
            ...
            {activeStep === 1 && (
              <div className="fade-in space-y-8">
                <div className="flex justify-between items-end mb-2">
                  <div>
                    <h2 className="text-3xl font-bold text-[var(--foreground)] mb-2">Identificar Transcripción</h2>
                    <p className="text-[var(--muted)]">Asigna un nombre y categoría para organizar tus archivos.</p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 bg-[var(--card)] p-8 rounded-2xl border border-[var(--border)] items-end">
                   <div className="space-y-3 flex flex-col">
                      <div className="h-5 flex items-center">
                        <label className="text-[10px] font-bold uppercase tracking-widest text-[var(--primary)]">Título Clase</label>
                      </div>
                      <input 
                        type="text" 
                   ...
                   <div className="space-y-3 flex flex-col relative">
                      <div className="h-5 flex justify-between items-center">
                        <label className="text-[10px] font-bold uppercase tracking-widest text-[var(--primary)] text-left">Ramo / Categoría</label>
                      ...
                      {!isAddingNewCategory ? (
                        <div className="relative">
                          ...
                          >
                            ...
                            <ChevronDown size={16} className={cn("transition-transform duration-300", isCategoryDropdownOpen && "rotate-180")} />
                          </button>
                          
                          {isCategoryDropdownOpen && (
                            <>
                              <div className="fixed inset-0 z-[60]" onClick={() => setIsCategoryDropdownOpen(false)}></div>
                              <div className="absolute top-[calc(100%+8px)] left-0 w-full bg-[var(--card)] border border-[var(--border)] rounded-xl shadow-2xl z-[70] overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200">
                                <div className="max-h-48 overflow-y-auto">
                                  {availableCategories.map(cat => (
                                    <div 
                                      ...
                                      className={cn(
                                        ...
                                        category === cat && "bg-[var(--primary)] bg-opacity-20 text-[var(--primary)] font-bold"
                                      )}
                                    >
                                      {cat}
                                    </div>
                                  ...
                                  <div 
                                    onClick={() => { setIsAddingNewCategory(true); setCategory(''); setIsCategoryDropdownOpen(false); }}
                        ...
                        <div className="relative">
                          <input 
                            ...
                            onBlur={() => { if(!category) { setIsAddingNewCategory(false); setCategory('General'); } }}
                   ...
                   <div className="space-y-3 flex flex-col">
                      <div className="h-5 flex items-center">
                        <label className="text-[10px] font-bold uppercase tracking-widest text-[var(--primary)]">Fecha de Clase</label>
                      </div>
                      <div className="relative group">
                        ...
                        />
                        <Calendar size={16} className="absolute right-4 top-1/2 -translate-y-1/2 text-[var(--muted)] pointer-events-none z-0" />
                      </div>
                   </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-12">
                  ...
                  <div 
                    ...
                    className={cn(
                      ...
                      audioPath ? "border-[var(--primary)] shadow-[0_0_20px_rgba(var(--primary-rgb),0.1)]" : "border-[var(--border)] hover:border-[var(--primary)]"
                    )}
                    onDragOver={(e) => e.preventDefault()}
                    onDrop={handleAudioDrop}
                  >
                    <div className={cn("p-6 rounded-2xl transition-all duration-500", audioPath ? "bg-[var(--primary)] text-white scale-110 shadow-lg" : "bg-[var(--secondary)] text-[var(--muted)]")}>
                      <FileAudio size={48} />
                    </div>
                    <div>
                      <h3 className="text-lg font-bold text-[var(--foreground)] tracking-tight">Archivo de Audio</h3>
                      <p className="text-xs text-[var(--muted)] mt-1 uppercase font-bold tracking-widest">MP3, WAV, M4A, FLAC</p>
                    </div>
                    {audioPath ? (
                      ...
                      <div className="flex flex-col items-center gap-3">
                        <p className="text-[11px] text-[var(--muted)] italic">Haz clic para buscar localmente</p>
                        <button 
                          onClick={(e) => { e.stopPropagation(); handleOpenDriveExplorer('audio'); }}
                  ...
                  <div 
                    ...
                    className={cn(
                      "premium-card p-10 flex flex-col items-center justify-center gap-4 text-center cursor-pointer border-dashed border-2 transition-all duration-300 bg-[var(--card)]",
                      contextPath ? "border-emerald-500 shadow-[0_0_20px_rgba(16,185,129,0.1)]" : "border-[var(--border)] hover:border-emerald-500"
                    )}
                  >
                    <div className={cn("p-6 rounded-2xl transition-all duration-500", contextPath ? "bg-emerald-600 text-white scale-110 shadow-lg" : "bg-[var(--secondary)] text-[var(--muted)]")}>
                      <Maximize2 size={48} />
                    </div>
                    <div>
                      <h3 className="text-lg font-bold text-[var(--foreground)] tracking-tight">Contexto Médico</h3>
                      <p className="text-xs text-[var(--muted)] mt-1 uppercase font-bold tracking-widest">PPTX o PDF</p>
                    </div>
                    {contextPath ? (
                      <div 
                      ...
                      <div className="flex flex-col items-center gap-3">
                         <p className="text-[11px] text-[var(--muted)] italic">Opcional para mejorar precisión</p>
                         <button 
                           onClick={(e) => { e.stopPropagation(); handleOpenDriveExplorer('context'); }}
                ...
                <div className="glass-morphism rounded-2xl p-8 space-y-6">
                  <h4 className="text-sm font-bold uppercase tracking-wider text-[var(--muted)]">Configuración del Modelo</h4>
                  <div className="flex flex-wrap gap-4">
                    {[
                      ...
                      <button 
                        key={lvl.id}
                        onClick={() => setQuality(lvl.id === 'turbo' ? 'large-v3-turbo' : lvl.id)}
                        className={cn(
                          "px-6 py-3 rounded-xl border-2 transition-all font-semibold capitalize text-sm",
                          (quality === lvl.id || (quality === 'large-v3-turbo' && lvl.id === 'turbo')) 
                  ...
                  <div className="flex items-start gap-3 p-4 bg-[var(--card)] rounded-lg border border-[var(--border)]">
                    <AlertCircle size={18} className="text-[var(--muted)] mt-0.5" />
                    <div className="space-y-1">
                      <p className="text-[13px] text-[var(--muted)] leading-relaxed italic">
                        La IA detectará automáticamente tu GPU para máxima velocidad.
                      </p>
                      <p className="text-[11px] text-[var(--muted)] opacity-70">
                        Modo Turbo requiere 4GB+ de VRAM. Si falla, el sistema usará CPU automáticamente.
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex justify-end pt-4">
                  <button 
                    ...
                    className={cn(
                      ...
                      (!audioPath || isProcessing) && "opacity-50 cursor-not-allowed"
                    )}
                  >
                ...
                <div className="relative">
                  ...
                  <div className="w-56 h-56 rounded-full border-4 border-[var(--border)] relative">
                    ...
                    <div className="absolute inset-0 flex flex-col items-center justify-center text-center">
                      <span className="text-5xl font-black text-[var(--foreground)]">{progress}%</span>
                      <span className="text-[10px] text-[var(--primary)] font-bold uppercase tracking-widest mt-2">{progress < 30 ? 'Cargando Modelo' : progress < 80 ? 'Analizando Audio' : 'Finalizando'}</span>
                    </div>
                  </div>
                </div>
 
                <div className="text-center space-y-4 max-w-sm">
                  <h3 className="text-xl font-bold text-[var(--foreground)]">Procesando audio veterinario...</h3>
                  <p className="text-[var(--muted)] text-sm">
                    No cierres el programa. Estamos utilizando tu GPU para mayor velocidad y privacidad absoluta.
                  </p>
                </div>
            ...
            {activeStep === 3 && (
              <div className="fade-in space-y-6">
                 <div className="flex items-center justify-between">
                  <div>
                    <h2 className="text-2xl font-bold text-[var(--foreground)]">Transcripción Finalizada</h2>
                    <p className="text-[var(--muted)] text-sm">Revisa el contenido antes de exportar. La IA ha detectado párrafos naturales.</p>
                  </div>
                  <div className="flex gap-3 relative">
                    ...
                    >
                      {saveStatus.includes('Copiado') ? <ClipboardCheck size={16} className="text-green-500" /> : <Clipboard size={16} />} 
                      COPIAR
                    </button>
                    <button 
                      ...
                      <div className="bg-[var(--secondary)] border-b border-[var(--border)] px-6 py-3 flex text-xs font-bold text-[var(--muted)] uppercase tracking-widest justify-between items-center">
                        <div className="flex items-center gap-4">
                           ...
                           <span className="text-[var(--primary)] flex items-center gap-1.5">
                             <CheckCircle2 size={12} /> {transcription.split(/\s+/).length} palabras
                           </span>
                ...
                <div className="flex items-center gap-2 text-xs text-[var(--muted)] italic bg-[var(--secondary)] p-3 rounded-lg border border-[var(--border)]">
                  ...
                  Los términos médicos como 'Adenitis' y 'Cardiomegalia' fueron reforzados por el motor de contexto local.
                </div>
              </div>
            )}
            ...
            {activeStep === 4 && (
              <div className="fade-in space-y-8">
                <div className="flex justify-between items-end mb-2">
                  <div>
                    <h2 className="text-3xl font-bold text-[var(--foreground)] mb-2">Biblioteca de Transcripciones</h2>
                    <p className="text-[var(--muted)]">Explora y recupera tus apuntes filtrando por Ramo y Fecha.</p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-[var(--card)] p-6 rounded-2xl border border-[var(--border)] items-end">
                   <div className="space-y-3 flex flex-col relative z-20">
                      <div className="h-5 flex items-center">
                        <label className="text-[10px] font-bold uppercase tracking-widest text-[var(--primary)] text-left">Filtrar por Ramo</label>
                      </div>
                      
                      <div className="relative">
                        <button 
                          onClick={() => setIsLibraryCategoryDropdownOpen(!isLibraryCategoryDropdownOpen)}
                          className="w-full h-[48px] bg-[var(--secondary)] border border-[var(--border)] rounded-xl px-4 text-sm flex justify-between items-center hover:border-[var(--primary)] transition-all outline-none"
                        >
                          <span className="font-medium">{selectedLibraryCategory}</span>
                          <ChevronDown size={16} className={cn("transition-transform duration-300", isLibraryCategoryDropdownOpen && "rotate-180")} />
                        </button>
                        
                        {isLibraryCategoryDropdownOpen && (
                          <>
                            <div className="fixed inset-0 z-[60]" onClick={() => setIsLibraryCategoryDropdownOpen(false)}></div>
                            <div className="absolute top-[calc(100%+8px)] left-0 w-full bg-[var(--card)] border border-[var(--border)] rounded-xl shadow-2xl z-[70] overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200">
                              <div className="max-h-48 overflow-y-auto">
                                {availableCategories.map(cat => (
                                  <div 
                                    key={`lib-cat-${cat}`} 
                                    onClick={() => { setSelectedLibraryCategory(cat); setIsLibraryCategoryDropdownOpen(false); setSelectedLibraryDay(null); }}
                                    className={cn(
                                      "px-4 py-3 text-sm cursor-pointer hover:bg-[var(--primary)] hover:text-white transition-colors",
                                      selectedLibraryCategory === cat && "bg-[var(--primary)] bg-opacity-20 text-[var(--primary)] font-bold"
                                    )}
                                  >
                                    {cat}
                                  </div>
                   ...
                   <div className="flex items-center justify-between bg-[var(--secondary)] border border-[var(--border)] rounded-xl h-[48px] px-2">
                     <button onClick={handlePrevMonth} className="p-2 hover:text-[var(--primary)] transition-colors rounded-lg hover:bg-[var(--primary)] hover:bg-opacity-10">
                       <ChevronLeft size={20} />
                     </button>
                     <div className="flex flex-col items-center">
                       <span className="font-bold text-[var(--foreground)] text-sm">{monthNames[libraryMonth]}</span>
                       <span className="text-[10px] text-[var(--muted)] font-black tracking-widest">{libraryYear}</span>
                     </div>
                     <button onClick={handleNextMonth} className="p-2 hover:text-[var(--primary)] transition-colors rounded-lg hover:bg-[var(--primary)] hover:bg-opacity-10">
                ...
                {!selectedLibraryDay && (
                  <div className="glass-morphism rounded-2xl p-8 shadow-xl relative z-10 border border-[var(--border)] animate-in fade-in duration-300">
                    <div className="grid grid-cols-7 gap-2 mb-4">
                      {weekDays.map(day => (
                        <div key={day} className="text-center text-[10px] font-bold text-[var(--muted)] uppercase tracking-widest">
                          {day}
                        </div>
                      ))}
                    </div>
                  ...
                  <div className="animate-in slide-in-from-right-4 fade-in duration-300 space-y-6">
                    <div className="flex items-center gap-4 border-b border-[var(--border)] pb-4">
                      ...
                      <h3 className="text-xl font-bold flex items-center gap-2 m-0 leading-none">
                         ...
                         Apuntes del {selectedLibraryDay} de {monthNames[libraryMonth]}
                      </h3>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                       {getItemsForDay(selectedLibraryDay).map((item: any, idx: number) => (
                           <div 
                             key={`result-${idx}`} 
      ...
      <footer className="px-8 py-2 bg-[var(--footer)] border-t border-[var(--border)] text-[10px] text-[var(--muted)] flex justify-between items-center z-50 text-left">
        ...
        <div className="flex items-center gap-2 px-2 py-1 bg-[var(--background)] rounded-lg border border-[var(--border)]">
          ...
          {[
            ...
            { id: 'light', color: '#f8fafc', icon: <Sun size={10} className="text-orange-500" /> },
            ...
            <button
              ...
              className={cn(
                ...
                theme === t.id ? "border-[var(--primary)] scale-110 shadow-lg" : "border-transparent opacity-50 hover:opacity-100"
              )}
        ...
        <div className="font-medium tracking-tight">
          <a href="https://github.com/mtynnn/vTransciptor" target="_blank" className="hover:text-blue-400 transition-colors">
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-[var(--card)] border border-[var(--border)] w-full max-w-2xl rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[80vh]">
            <div className="p-6 border-b border-[var(--border)] flex justify-between items-center bg-[var(--header)]">
              <h3 className="text-xl font-bold text-[var(--foreground)]">Historial de Transcripciones</h3>
              <button onClick={() => setShowHistory(false)} className="text-[var(--muted)] hover:text-[var(--foreground)] text-2xl">&times;</button>
            </div>
            <div className="flex-1 overflow-y-auto p-4 space-y-3">
              {historyItems.length === 0 ? (
                <p className="text-center text-[var(--muted)] py-10 italic">No hay transcripciones previas disponibles.</p>
              ) : (
                historyItems.map((item, idx) => (
                  ...
                  >
                    <div className="flex justify-between items-start mb-3">
                       ...
                       <span className="text-[var(--muted)] text-[10px] font-medium">{item.date}</span>
                    </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-black/60 backdrop-blur-sm animate-in zoom-in duration-200">
          <div className="bg-[var(--card)] border border-[var(--border)] w-full max-w-md rounded-2xl shadow-2xl p-8 space-y-6">
            <div className="flex justify-between items-center mb-2">
              <h3 className="text-xl font-bold text-[var(--foreground)]">Configuración General</h3>
              <button onClick={() => setShowSettings(false)} className="text-[var(--muted)] hover:text-[var(--foreground)] text-2xl">&times;</button>
            </div>
            <div className="space-y-4">
              <div className="space-y-2 text-left">
                <label className="text-xs font-bold text-[var(--muted)] uppercase">Modelo Predeterminado</label>
                <select 
                ...
                >
                  ...
                  <option value="large-v3-turbo">Máxima Precisión (Turbo)</option>
                </select>
              </div>
              ...
              <div className="space-y-3 text-left">
                <label className="text-xs font-bold text-[var(--muted)] uppercase flex items-center gap-2">
                  <Library size={14} className="text-[var(--primary)]" /> Google Drive (Nube)
                </label>
                <div className="flex flex-col gap-2">
                   {driveAccounts.length > 0 ? (
                     driveAccounts.map(acc => (
                       <div key={acc} className="flex items-center justify-between bg-[var(--background)] border border-[var(--border)] rounded-xl p-3 text-[11px]">
                          <span className="font-bold flex items-center gap-2">
                             <div className="w-2 h-2 rounded-full bg-green-500 shadow-[0_0_5px_rgba(34,197,94,0.5)]"></div> {acc}
                          </span>
                       </div>
                     ))
                   ) : (
                     <p className="text-[10px] text-[var(--muted)] italic mb-1 px-1">Sin cuentas vinculadas aún.</p>
                   )}
                   <button 
                   ...
                   >
                     {isLinkingAccount ? <Loader2 className="animate-spin" size={14} /> : (
                       <>
              ...
              <div className="flex items-center justify-between py-2">
                 <span className="text-[11px] font-bold text-[var(--muted)] uppercase tracking-widest">Auto-Guardado TXT</span>
           ...
           <div className="bg-[var(--card)] border border-[var(--border)] w-full max-w-3xl rounded-3xl shadow-[0_0_50px_rgba(0,0,0,0.5)] overflow-hidden flex flex-col max-h-[85vh]">
              <div className="p-6 border-b border-[var(--border)] flex justify-between items-center bg-[var(--header)]">
                 <div className="flex items-center gap-4">
                    ...
                    <div>
                       <h3 className="text-xl font-bold text-[var(--foreground)]">Explorador de Google Drive</h3>
                       <div className="flex items-center gap-2 mt-1">
                          <select 
                            ...
                            onChange={(e) => { setSelectedDriveAccount(e.target.value); fetchDriveFiles('root'); }}
                          >
                             {driveAccounts.map(acc => <option key={acc} value={acc} className="bg-[var(--card)]">{acc}</option>)}
                          </select>
                       </div>
                    </div>
                 </div>
                 <button onClick={() => setIsDriveModalOpen(false)} className="text-[var(--muted)] hover:text-[var(--foreground)] text-3xl font-light">&times;</button>
              </div>
                   ...
                   <React.Fragment key={crumb.id}>
                      <button 
                        ...
                        className={cn(
                          ...
                          idx === driveBreadcrumbs.length - 1 ? "text-[var(--primary)]" : "text-[var(--muted)] hover:text-[var(--foreground)]"
                        )}
                      >
                        {crumb.name}
                      </button>
                      {idx < driveBreadcrumbs.length - 1 && <span className="text-[var(--muted)] opacity-30">/</span>}
                   </React.Fragment>
                 ))}
              </div>
              ...
              <div className="flex-1 overflow-y-auto p-6 custom-scrollbar bg-[var(--background)]">
                 {isDriveLoading ? (
                   <div className="h-full flex flex-col items-center justify-center space-y-4 py-20">
                      ...
                      <p className="text-[var(--muted)] text-[10px] animate-pulse tracking-[0.2em] uppercase font-black">Sincronizando con Google Cloud...</p>
                   </div>
                 ) : driveError ? (
                   <div className="h-full flex flex-col items-center justify-center space-y-4 py-20 text-center px-10">
                      ...
                      <p className="text-red-400 font-bold text-sm tracking-tight">{driveError}</p>
                      <button onClick={() => fetchDriveFiles(driveCurrentFolderId)} className="text-[10px] uppercase font-bold text-[var(--primary)] hover:underline">Reintentar</button>
                   </div>
                 ) : (
                   <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 pb-10 fade-in">
                      ...
                      {driveFiles.filter(f => f.mimeType === 'application/vnd.google-apps.folder').map(folder => (
                        <div 
                          key={folder.id}
                          onClick={() => navigateToDriveFolder(folder.id, folder.name)}
                        ...
                        >
                           <Library className="mx-auto text-yellow-500 group-hover:scale-110 transition-transform" size={40} />
                           <span className="text-[11px] font-bold truncate block px-2 leading-relaxed text-[var(--foreground)]">{folder.name}</span>
                        </div>
                      ))}
                      {/* Archivos Después */}
                      {driveFiles.filter(f => f.mimeType !== 'application/vnd.google-apps.folder').map(file => {
                        const isSelectable = driveTargetType === 'audio' 
                          ? file.mimeType.includes('audio') || file.mimeType.includes('video') || file.name.match(/\.(mp3|wav|m4a|flac)$/i)
                          : file.name.match(/\.(pptx|pdf|docx|txt)$/i) || file.mimeType.includes('presentation') || file.mimeType.includes('pdf');
                          
                        return (
                          <div 
                            key={file.id}
                            onClick={() => isSelectable && handleSelectDriveFile(file)}
                            className={cn(
                          ...
                          >
                             {file.mimeType.includes('audio') ? (
                               <FileAudio className="mx-auto text-[var(--primary)] group-hover:scale-110 transition-transform" size={40} />
                             ) : file.mimeType.includes('presentation') || file.name.toLowerCase().endsWith('.pptx') ? (
                               <Maximize2 className="mx-auto text-orange-500 group-hover:scale-110 transition-transform" size={40} />
                             ) : (
                               <FileText className="mx-auto text-red-500 group-hover:scale-110 transition-transform" size={40} />
                             )}
                             <span className="text-[10px] font-bold truncate block px-2 text-[var(--foreground)]">{file.name}</span>
                          </div>
                        );
                      })}
                      {driveFiles.length === 0 && (
                        <div className="col-span-full py-20 text-center">
                          <p className="text-[var(--muted)] text-sm italic py-10">Esta carpeta no contiene archivos compatibles.</p>
                        </div>
                      )}
                   </div>
                 )}
        ...
        <div className="fixed inset-0 z-[250] flex items-center justify-center p-6 bg-black/80 backdrop-blur-md animate-in fade-in duration-200">
           <div className="bg-[var(--card)] border border-[var(--border)] w-full max-w-sm rounded-3xl shadow-[0_0_50px_rgba(var(--primary-rgb),0.2)] p-8 space-y-6 animate-in zoom-in-95 duration-200">
              <div className="text-center space-y-2">
                 ...
                 <h3 className="text-xl font-bold text-[var(--foreground)]">Identifica tu cuenta</h3>
                 <p className="text-[10px] text-[var(--muted)] uppercase font-bold tracking-widest">Google Drive Integration</p>
              </div>
              
              <div className="space-y-3">
                 <label className="text-[10px] font-black text-[var(--primary)] uppercase tracking-wider ml-1">Nombre de la cuenta</label>
                 <input 
                   autoFocus
                   type="text" 
                   value={accountNameInput}
                   onChange={(e) => setAccountNameInput(e.target.value)}
                   onKeyDown={(e) => e.key === 'Enter' && confirmLinkDriveAccount()}
